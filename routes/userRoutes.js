...
router.put('/additemtocart/:id', authentication, async (req, res) => {
    ...
    const itemWithQuantity = { id: uuid.v4(), item, quantity: 1 }
    //  = await Users.findById(req.user.id)
    const user = await Users.findByIdAndUpdate(req.user.id, { $push: { cartItem: itemWithQuantity } })
    // user.cartItem.push(item)
    // user.save()
    res.json(user)
})

router.put('/increaseitemquantity/:cartItemId', authentication, async (req, res) => {
    const user = await Users.findById(req.user.id)
    // const user = await Users.findByIdAndUpdate(req.user.id, { $set: { cartItem: cartItem } })
    const newUser = await Users.findByIdAndUpdate(req.user.id, {
        $set: {
            cartItem: user.cartItem.map(element => {
                // console.log(element);
                return (
                    element.id === req.params.cartItemId
...
router.put('/decreaseitemquantity/:cartItemId', authentication, async (req, res) => {
    const user = await Users.findById(req.user.id)
    // const user = await Users.findByIdAndUpdate(req.user.id, { $set: { cartItem: cartItem } })
    const newUser = await Users.findByIdAndUpdate(req.user.id, {
        $set: {
...
router.delete('/deleteitemfromcart/:id', authentication, async (req, res) => {
    //  = await Users.findById(req.user.id)
    const user = await Users.findById(req.user.id)
    const item = user.cartItem.find(element => element.id === req.params.id)
    const newUser = await Users.findByIdAndUpdate(req.user.id, { $pull: { cartItem: item } }, { multi: false })
    // user.cartItem.push(item)
    // user.save()
    res.json(newUser)
})
